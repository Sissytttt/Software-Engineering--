...
def registerAuth_staff():
    ...
    if (data):
        ...
        cursor.execute(ins, (username, password, first_name, last_name, birth, airline))
        conn.commit()
        cursor.close()
        return render_template('index.html')
...
def agent_display_purchased():
    ...
    query = "SELECT  p.purchase_date, t.ticket_id, f.airline_name, f.flight_num, f.departure_airport, f.departure_time, " \
            "f.arrival_airport, f.arrival_time, f.price, f.status, f.airplane_id from ticket as t, flight as f, " \
            "purchases as p where p.booking_agent_id = %s and p.ticket_id = t.ticket_id and t.flight_num =  " \
            "f.flight_num order by p.purchase_date ASC "
...
def agent_purchase_ticket():
    ...
    if (airline):
        ...
        if current_occupied[0]["c"] < available[0]["seats"]:
            ...
            ins1 = 'INSERT INTO purchases VALUES(%s, %s, %s, CURRENT_DATE())'
            cursor.execute(ins1, (str(ticket_id + 1), username, agent_id))
            conn.commit()
            cursor.close()
...
def agent_commission():
    ...
    query_money = 'SELECT price FROM flight NATURAL JOIN ticket NATURAL JOIN purchases NATURAL JOIN booking_agent WHERE ' \
                  'booking_agent_id = %s AND purchase_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 1 MONTH) AND CURDATE() '
    ...
    query_count = 'SELECT count(ticket_id) as c FROM flight NATURAL JOIN ticket NATURAL JOIN purchases NATURAL JOIN booking_agent WHERE ' \
                  'booking_agent_id = %s AND purchase_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 1 MONTH) AND CURDATE() '
...
def agent_customize_commission():
    ...
    query_money_2 = 'SELECT price FROM flight NATURAL JOIN ticket NATURAL JOIN purchases NATURAL JOIN booking_agent WHERE ' \
                    'booking_agent_id = %s AND purchase_date BETWEEN %s AND %s '
    ...
    for item in commission_c:
        total_commission_c += int(item["price"])
    avg_commission_c = str(int(total_commission_c / 30))
    total_commission_c = str(total_commission)

    query_count_2 = 'SELECT count(ticket_id) as c FROM flight NATURAL JOIN ticket NATURAL JOIN purchases NATURAL JOIN booking_agent WHERE ' \
                    'booking_agent_id = %s AND purchase_date BETWEEN %s AND %s '
...
def agent_top_customer():
    ...
    query_money = 'SELECT sum(price) as cp, customer_email FROM flight NATURAL JOIN ticket NATURAL JOIN ' \
                  'purchases NATURAL JOIN booking_agent WHERE ' \
                  'booking_agent_id = %s and purchase_date between DATE_SUB(CURDATE(), INTERVAL 6 MONTH) AND ' \
    ...
    query_count = 'SELECT count(ticket_id) as ct, customer_email as c FROM flight NATURAL JOIN ticket NATURAL JOIN purchases NATURAL JOIN booking_agent WHERE ' \
...
def loginAuth_staff():
    ...
    error = None
    # if data is not none
    if data is not None and identity == "airline_staff":
        # creates a session for the the user
        # 创造一个会话
        # session is a built in
        session['username'] = username
...
def staff_add_airplane_form():
    ...
    try:
        ...
        flash('You have successfully add an airplane!')
        return render_template('staff_page/staff_add_airplane.html', airline = session['airline'], airplanes = airplanes)

    except:
        flash('Failed! Invalid input or the airplane already exists!')
        return render_template('staff_page/staff_add_airplane.html', airline = session['airline'])
...
def staff_create_flight_form():
    ...
    try:
        ...
        inserting = "insert into flight VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s)"
        cursor.execute(inserting, (airline, int(flight_num), departure_airport, departure_time,arrival_airport, arrival_time, price, status, airplane_id))
        conn.commit()
        cursor.close()
...
def staff_change_status():
    ...
    if identity != 'airline_staff':
        flash("Sorry! You are not allowed to change status!")
        return render_template('staff_page/staff_home.html')
...
def staff_add_agent_form():
    ...
    try:
        ...
        cursor.execute(query, (agent_email_address, agent_password, int(agent_id)))
        conn.commit()
    
        cursor = conn.cursor()
        query = "insert into booking_agent_work_for values (%s, %s)"
        cursor.execute(query, (agent_email_address, airline))
        conn.commit()
        cursor.close()
        flash('You have successfully add an agent!')
        return render_template('staff_page/staff_add_agent.html',  airline = airline_1)
    except:
        flash('Failed! Invalid input or that agent already exists!')
        return render_template('staff_page/staff_add_agent.html', airline = airline_1)
...
def staff_view_top_agent():
    ...
    query = "select b.email, b.booking_agent_id, count(DISTINCT p.ticket_id) " \
            "from booking_agent as b, purchases as p " \
            "where p.purchase_date >= adddate(date(now()), interval -1 month) " \
            "and b.booking_agent_id = p.booking_agent_id " \
...
def staff_view_freq_customer():
    ...
    cursor = conn.cursor()
    query =   "select customer.email, customer.name " \
	        "from ticket natural join flight natural join purchases join customer on (purchases.customer_email = customer.email)  " \
            "where flight.airline_name = %s "   \
	        "group by customer.email " \
	        "having COUNT(ticket.ticket_id) >= all ( select COUNT(ticket.ticket_id) " \
            "from ticket natural join flight natural join purchases join customer on (purchases.customer_email = customer.email) " \
            "group by customer.email  )  "
    cursor.execute(query, (airline["airline_name"]))
...
def staff_view_all_flights_of_customer():
        ...
        query = "select * " \
                "from flight as f, purchases as p, ticket as t " \
                "where p.customer_email = %s and p.ticket_id = t.ticket_id " \
                "and t.flight_num = f.flight_num and f.airline_name = %s"
...
def staff_view_Top_destinations():
        ...
        query = "select f.arrival_airport " \
                    ...
                    "and t.flight_num = f.flight_num and f.airline_name = %s and " \
                    "(f.arrival_time between adddate(now(), interval -3 month) and now()) " \
                    "and a.airport_name = f.arrival_airport " \
                    "group by f.arrival_airport order by count(p.customer_email) desc"
        ...
        query = "select f.arrival_airport " \
                    ...
                    "where p.ticket_id = t.ticket_id and t.airline_name = f.airline_name " \
                    "and t.flight_num = f.flight_num and f.airline_name = %s and " \
                    "(f.arrival_time between date_sub(now(),interval 12 month) and now() ) " \
                    "and a.airport_name = f.arrival_airport " \
                    "group by f.arrival_airport order by count(p.customer_email) desc"
...
def staff_view_customer_of_flight():
        ...
        if identity != 'airline_staff':
            flash("Sorry! You are not allowed to check the flights")
        ...
        query = "select c.email, c.name" \
                " from ticket as t,  purchases as p, customer as c " \
                "where c.email = p.customer_email  and  t.ticket_id = p.ticket_id  and  t.airline_name = %s and t.flight_num = %s"
        
        cursor.execute(query, (airline['airline_name'], int(flight_number)))
...
def staff_view_my_flights():
    ...
    cursor.close()
    return render_template('/staff_page/staff_view_my_flights.html', flights = flights,
                                                                    arrival_airport = arrival_airport,
                                                                    departure_airport = departure_airport,
...
def staff_search():
    ...
    if check1 == 'Use_airport_to_search':
        ...
        cursor = conn.cursor()
        query = 'SELECT * from flight as f, airport as a1, airport as a2 ' \
...
def staff_comparision_of_reveneue_earned():
    ...
    query = "select sum(f.price) " \
            "from purchases as p, flight as f, ticket as t " \
            "where (p.purchase_date >= adddate(date(now()), interval -1 month)) " \
            "and f.airline_name = %s " \
            "and t.flight_num = f.flight_num " \
            "and t.ticket_id = p.ticket_id  and p.booking_agent_id is null"
    cursor.execute(query, (airline['airline_name']))
    one_month_revenue_direct = cursor.fetchone()
    print(one_month_revenue_direct)
    ...
    query = "select sum(f.price) " \
            ...
            "and f.airline_name = %s " \
            "and t.flight_num = f.flight_num " \
            "and t.ticket_id = p.ticket_id  and p.booking_agent_id is not null"
    cursor.execute(query, (airline['airline_name']))
...
"where `date` between %s and %s; "
    ...
    for i in range(len(all_month)-1):
        ...
        cursor = conn.cursor()
        query = "select COUNT(*) " \
                "from purchases NATURAL JOIN flight NATURAL JOIN ticket " \
                "where airline_name=%s and purchases.purchase_date >= %s and purchases.purchase_date < %s "
